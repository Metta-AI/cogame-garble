...
# Substitute: garble, coworld-garble, 5.
#
...
#
# env:
#   SMOKE_IMAGE                image, if not given as $1        (coworld-garble:ci)
#   SMOKE_SLUG                 game slug                        (garble)
#   SMOKE_GAME_BIN             game entrypoint                  (/bin/garble)
#   SMOKE_PLAYER_BIN           player entrypoint                (/bin/garble-player)
...
image="${1:-${SMOKE_IMAGE:-coworld-garble:ci}}"
slug="${SMOKE_SLUG:-garble}"
...
# --------------------------------------------------------------------------
# Every PLAYER container must exit 0 too, not just the game (raid 0.1.3 ->
# 0.1.4: whisky's receiveMessage RAISES on a close frame, so a player that
# does not wrap its receive loop dies non-zero after a perfectly good
# episode -- invisible to a smoke that only inspects ${prefix}-game).
# The players are given a grace window to notice the game is gone.
# --------------------------------------------------------------------------
...
for ((slot = 0; slot < seats; slot++)); do
  ...
  while docker ps -q --filter "name=${name}" | grep -q .; do
    if (( SECONDS > player_deadline )); then
      echo "FAIL: player container ${name} did not exit within 60s of the game" >&2
      dump_logs
      exit 1
    fi
    sleep 2
  done
  player_exit="$(docker inspect -f '{{.State.ExitCode}}' "${name}")"
  if [ "${player_exit}" != "0" ]; then
    echo "FAIL: player container ${name} exited ${player_exit}" >&2
    dump_logs
    exit 1
  fi
done
echo "every player container exited 0"
