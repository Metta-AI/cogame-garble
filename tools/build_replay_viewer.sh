...
# checkout does not, and the inherited hook then exits 1 on the first copy
# (ecos, 2026-08-23). Create the parent before anything looks at it.
...
test -s "${dist}/garble_replay.wasm"
test -s "${dist}/garble_replay.js"

rm -rf "${output_dir}"
mkdir -p "${output_dir}/assets"
cp "${dist}/garble_replay.js" "${dist}/garble_replay.wasm" "${output_dir}/"
cp "${repo_dir}/replay-viewer/index.html" \
  ...
  "${output_dir}/"
for asset in cog_red_front.png cog_blue_front.png \
...
echo "garble replay viewer bundle: ${output_dir}"
