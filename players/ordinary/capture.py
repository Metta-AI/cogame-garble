...
class Capture:
    ...
    def record(self, system: str, user: str, action: dict, source: str, turn: int) -> None:
        self.rows.append({
            "decision_id": len(self.rows), "turn": turn, "source": source,
    ...
    def upload(self, scores: list[float]) -> None:
        ...
        with zipfile.ZipFile(archive, "w", compression=zipfile.ZIP_DEFLATED) as output:
            output.writestr("trajectory.json", json.dumps({
                "schema_version": 1, "game": "garble", "slot": self.slot,
