"""Export accepted ordinary-player decisions from complete Garble episodes."""
...
def export(runs: list[Path], output: Path, revision: str, source: str,
    ...
    for run in runs:
        ...
        for artifact in sorted(run.glob("policy_artifact_*.zip")):
            ...
            if (trajectory["schema_version"] != 1 or trajectory["game"] != "garble"
            ...
            for row in trajectory["decisions"]:
                if row["source"] != source:
                    continue
                example = {
                    "episode_id": f"garble-{seed}-seat-{slot}",
                    "seed": f"garble-{seed}",
    ...
    for split, rows in splits.items():
        ...
        with os.fdopen(os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), "w") as destination:
            destination.write("\n".join(rows) + "\n")
    manifest = {"schema_version": 1, "game": "garble", "source_revision": revision,
                "action_schema_revision": "garble-decision-v1", "teacher": source,
                "train_examples": len(splits["train"]),
                "validation_examples": len(splits["validation"]), "episodes": episodes}
...
if __name__ == "__main__":
    ...
    print(export(directories, args.output, args.source_revision, args.source,
